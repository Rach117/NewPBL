...
<div class="p-8 max-w-6xl mx-auto">
    ...
    <div class="mb-10 text-center">
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Buat Usulan Baru</h1>
        <p class="text-slate-500 mt-2">Lengkapi formulir di bawah ini untuk mengajukan anggaran kegiatan.</p>
    </div>
    ...
    <form id="usulanForm" method="post" action="/usulan/create" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden">
            ...
            <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nama_kegiatan" required class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all font-medium" placeholder="Contoh: Workshop Teknologi 4.0">
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Penerima Manfaat</label>
                    <input type="text" name="penerima_manfaat" required class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all" placeholder="Contoh: 50 Mahasiswa TI">
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Target Luaran</label>
                    <input type="text" name="target_luaran" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all" placeholder="Contoh: Sertifikat Kompetensi">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Gambaran Umum & Latar Belakang</label>
                    <textarea name="gambaran_umum" required rows="4" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all" placeholder="Jelaskan mengapa kegiatan ini penting..."></textarea>
                </div>
            </div>
        </div>
            ...
            <div class="p-8">
                ...
                <select name="iku_id[]" multiple required class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 h-64 text-sm">
                    <?php foreach ($iku as $i): ?>
                        <option value="<?php echo $i['id']; ?>" class="p-3 border-b border-slate-50 hover:bg-blue-50 cursor-pointer rounded">
                            • <?php echo htmlspecialchars($i['deskripsi_iku']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <p class="text-xs text-slate-400 mt-2">* Tahan tombol CTRL untuk memilih lebih dari satu.</p>
            </div>
        </div>
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody id="rabBody" class="divide-y divide-slate-100 bg-white">
                        <tr class="group hover:bg-slate-50 transition-colors">
                            ...
                            <td class="p-3"><input type="text" name="uraian[]" class="w-full border-slate-200 rounded-lg text-sm focus:border-blue-500 focus:ring-0" required placeholder="Nama item..."></td>
                            <td class="p-3"><input type="number" name="volume[]" class="w-full border-slate-200 rounded-lg text-sm focus:border-blue-500 focus:ring-0 volume text-center" min="1" value="1" required></td>
                            <td class="p-3"><input type="text" name="satuan[]" class="w-full border-slate-200 rounded-lg text-sm focus:border-blue-500 focus:ring-0 text-center" required placeholder="Pcs/Rim"></td>
                            <td class="p-3"><input type="number" name="harga_satuan[]" class="w-full border-slate-200 rounded-lg text-sm focus:border-blue-500 focus:ring-0 harga_satuan text-right" min="0" required placeholder="0"></td>
                            <td class="p-3"><input type="text" name="total[]" class="w-full bg-slate-100 border-transparent rounded-lg text-sm font-bold text-slate-700 total cursor-not-allowed text-right" readonly></td>
                            <td class="p-3 text-center"><button type="button" class="removeRow text-slate-300 hover:text-rose-500 transition-colors p-2 rounded-full hover:bg-rose-50"><span class="material-icons text-lg">delete</span></button></td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div class="flex items-center justify-end gap-4 pt-6 pb-20">
            ...
            <button type="submit" class="px-8 py-3 bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-700/30 hover:bg-blue-800 hover:-translate-y-1 transition-all flex items-center">
                <span class="material-icons text-sm mr-2">save</span> Simpan & Ajukan
            </button>
        </div>
    </form>
</div>
